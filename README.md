...
![Сайт Госуслуг с кнопками скачивания](./assets/Pasted%20image%2020251112152933.png)

Поэтому я набросал простой скрипт для Tampermonkey, чтобы перехватить запрос на скачивание.

> Скрипт можно найти в репозитории: `[./scripts/getlink.user.js](./scripts/getlink.user.js)`

После запуска скрипт показал, что кнопка не ведет на файл напрямую, а вызывает код из вот этого JavaScript-файла:

![Консоль с выводом скрипта](./assets/Pasted%20image%2020251112153241.png)
...
![Обфусцированный JS-код](./assets/Pasted%20image%2020251112153540.png)
...
> **Примечание:** Эти ==ссылки могут со временем умереть==. Для автоматизации поиска можно использовать скрипт ==(который позже тоже может умереть)== `[./scripts/deob.go](./scripts/deob.go)`. Но все нужные файлы уже лежат в этом репозитории в папке `[./certs](./certs)`.
...
![Терминал с выводом wget](./assets/Pasted%20image%2020251112154302.png)
...
![Терминал с выводом unzip](./assets/Pasted%20image%2020251112154402.png)
...
![Вывод команды openssl](./assets/Pasted%20image%2020251112154803.png)

Но если разобраться, все становится ясно. Полные протоколы допроса на момент написания статьи лежат здесь:
- `[./certs/interrogation/root_rsa.txt](./certs/interrogation/root_rsa.txt)`
- `[./certs/interrogation/sub_rsa.txt](./certs/interrogation/sub_rsa.txt)`
- `[./certs/interrogation/root_gost.txt](./certs/interrogation/root_gost.txt)`
- `[./certs/interrogation/sub_gost.txt](./certs/interrogation/sub_gost.txt)`
...
![Скриншот с сайта Госуслуг](./assets/Pasted%20image%2020251112163450.png)
...
![Скриншот с сайта Госуслуг](./assets/Pasted%20image%2020251112163945.png)
...
![Скриншот с сайта Госуслуг](./assets/Pasted%20image%2020251112164304.png)
...
![Скриншот с сайта Госуслуг](./assets/Pasted%20image%2020251112164658.png)
...
**Финальный вывод:** Угроза MitM-атаки при установке этих сертификатов — не злой сон а реальность,  Она **заложена в самой архитектуре этой системы**. Владелец корневого сертификата по определению получает власть над вашим трафиком.
